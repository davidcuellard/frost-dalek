...
impl SecretKey {
    ...
    /// Serialize `SecretKey` to bytes (index and scalar key).
    pub fn to_bytes(&self) -> ([u8; 32], u32) {
        (self.key.to_bytes(), self.index)
    }
...
mod test {
    ...
    fn reconstruct_secret(participants: &Vec<&DealtParticipant>) -> Result<Scalar, &'static str> {
        ...
        for this_participant in participants {
            let my_coeff = calculate_lagrange_coefficients(
...
fn test_secret_key_serialization() {
    ...
    // GroupKey test
    let group_key = GroupKey((&RISTRETTO_BASEPOINT_TABLE * &scalar));
    let serialized_group = group_key.to_bytes();
    let deserialized_group = GroupKey::from_bytes(serialized_group).unwrap();

    assert_eq!(group_key.0.compress(), deserialized_group.0.compress());
}
